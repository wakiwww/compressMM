...
| 优先级 | 问题                                           | 解决方案                                                        |
| ------ | ---------------------------------------------- | --------------------------------------------------------------- |
| 🔴 P0  | `VideoProcessor` 跨线程数据竞争                | 将`VideoProcessor` 改为 `@MainActor` 类，或使用 `actor`         |
| 🔴 P0  | `isCancelled`/`audioProcessingActive` 并发读写 | 使用`nonisolated(unsafe)` + `OSAllocatedUnfairLock` 或 `Atomic` |
| 🔴 P1  | `AVAsset.duration` 同步调用                    | 改为`async/await load(.duration)`                               |
| 🟡 P2  | `SafeCIContext` 的 `do-catch` 无效             | 移除多余的 do-catch                                             |
| 🟡 P2  | `UIScreen.main` 非主线程调用                   | 包装在`DispatchQueue.main.async` 中                             |
